...
### Tests (added 2026-07-19, pre-tag)
- PEP 440 version check accepts `.devN` tails without a phase marker (hook-derived stamps at fresh checkouts carry them).
- The densified-rank serialization seam documented as a repro (`tests/one-offs/`): `densify_between` promotes every rank to `Fraction`, including whole-number mediants -- JSON serialization downstream breaks; fix scheduled with the countable-christening work.
- The nucleus/rank-addressing human checklist joins `tests/checklists/`.
